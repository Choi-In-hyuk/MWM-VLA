...
#
# 결과: results/eval_plus/<suite>_denoise_stateinit/results.jsonl (resume 지원: 축/suite
...
CK=results/denoise_stateinit/stage2/checkpoints/mamba_wm_final.pt
...
run_one () {   # $1=suite  $2=category
  local SUITE=$1 CAT=$2
  local EOUT=results/eval_plus/${SUITE}_denoise_stateinit
...
echo "  suite별 결과: results/eval_plus/<suite>_denoise_stateinit/results.jsonl"
...
for f in glob.glob("results/eval_plus/*_denoise_stateinit/results.jsonl"):
    suite = f.split("/")[-2].replace("_denoise_stateinit","")
    for line in open(f):
