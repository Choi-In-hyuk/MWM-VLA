#!/bin/bash
# LIBERO fine-tune from PILOT-pretrained predictor (Phase 1 verification).
#
# Same as run_finetune_libero_from_pretrain.sh but points at the pilot ckpt.
# 30k step stage2 + 4-suite eval.
#
# Success signal:
#   * LIBERO 4-suite Avg > 93.7% (baseline stream_libero_all) -> GO Phase 2
#   * <= 93.7% -> STOP, reconsider recipe
#
# Usage:
#   bash scripts/run_finetune_libero_from_pilot.sh [stage2|eval|all]
set -eo pipefail
cd "$(dirname "$0")/.."
...
MIX=libero_all
TAG=finetune_libero_from_pilot
OUT=results/${TAG}
PRETRAIN_CKPT=results/pretrain_ssv2_droid_pilot/checkpoints/mamba_wm_final.pt

STAGE2_STEPS=30000
WARMUP_STEPS=2000
PER_GPU_BS=16          # LIBERO fine-tune keeps original per-gpu bs (recipe parity)
NPROC=2
EVAL_TRIALS=50
...
if [[ "$WHICH" == "all" || "$WHICH" == "stage2" ]]; then
  if [[ ! -f "${PRETRAIN_CKPT}" ]]; then
    echo "ERROR: pilot pretrain checkpoint not found: ${PRETRAIN_CKPT}"
    echo "Run scripts/run_pretrain_ssv2_droid_pilot.sh first."
    exit 1
  fi
  echo "=== Stage 2 (DDP x${NPROC}) from PILOT PRETRAIN: bs=${PER_GPU_BS}, ${STAGE2_STEPS} steps ==="
...
echo "=== done. Compare LIBERO Avg vs baseline 93.7% to decide Phase 2 go/no-go ==="
