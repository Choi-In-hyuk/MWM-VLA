#!/bin/bash
# LIBERO fine-tune from SSv2+Droid pretrained predictor.
#
# Skips stage1 (predictor is already trained by run_pretrain_ssv2_droid.sh),
# runs stage2 (predictor+action-head+LoRA+cond_proj) on libero_all,
# then 4-suite eval.
#
# Usage:
#   bash scripts/run_finetune_libero_from_pretrain.sh [stage2|eval|all]
set -eo pipefail
cd "$(dirname "$0")/.."
...
MIX=libero_all
TAG=finetune_libero_from_pretrain
OUT=results/${TAG}
...
if [[ "$WHICH" == "all" || "$WHICH" == "stage2" ]]; then
  if [[ ! -f "${PRETRAIN_CKPT}" ]]; then
    ...
    echo "Run scripts/run_pretrain_ssv2_droid.sh first."
    exit 1
  fi
